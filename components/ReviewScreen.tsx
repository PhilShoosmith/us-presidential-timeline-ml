...
const ReviewScreen: React.FC<ReviewScreenProps> = ({ presidents, onBack }) => {
  ...
  const handleLearnMore = useCallback(async (president: President) => {
    ...
    try {
      ...
      setRagContent({ title: president.name, text: t('rag.errorDetailed') + " " + (error?.message || String(error)), imageUrl: president.imageUrl });
    } finally {
      ...
      {areFiltersActive ? (
        <div className="w-full overflow-y-auto px-4 pt-4 pb-4 flex-grow">
          {filteredPresidents.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-8 md:gap-12 max-w-[120rem] mx-auto px-4 md:px-12">
